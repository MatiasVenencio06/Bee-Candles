...
function Bombones({condicional}) {
  ...
  useEffect(() => {
    ...
    const q = query(ItemsPersonalized, where('categoria', '==', 'bombones'))
  ...
  useEffect(() => {
    ...
    return (
        ...
        >
          ...
          {contextHolder}
            <Title style={{display: 'flex', justifyContent: 'center', marginTop: '1rem'}}>{`${condicional ? '' : 'Bombones aromaticos'}`}</Title>
