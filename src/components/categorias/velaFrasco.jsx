...
function VelasFrasco({condicional}) {
  ...
  useEffect(() => {
    ...
    const q = query(ItemsPersonalized, where('categoria', '==', 'frasco'))
  ...
  useEffect(() => {
    ...
    return (
        ...
        >
          ...
          {contextHolder}
            <Title style={{display: 'flex', justifyContent: 'center', marginTop: '1rem'}}>{`${condicional ? '' : 'Velas en frasco'}`}</Title>
